...
(defun hex-to-number (hex)
  "Convert `HEX' (a hex string) to a number."
  (string-to-number hex 16))

(defun number-to-2-digit-hex (number)
  "Convert `NUMBER' to a 2-digit hex string."
  (format "%02X" number))

(defun color-decomp (color)
  "Decompose a `COLOR' into a list."
  (list
   (hex-to-number (substring color 1 3))
   (hex-to-number (substring color 3 5))
   (hex-to-number (substring color 5 7))))

(defun color-recomp (color-list)
  "Re-compose a `COLOR-LIST' into a color string."
  (apply (apply-partially 'concat "#")
         (mapcar 'number-to-2-digit-hex color-list)))

(defun color-scale (color diff)
  "Scale `COLOR' by `DIFF'."
  (if diff
      (let* ((amount (* 16 (if (= (length diff) 3)
        ...
        (color-recomp (mapcar (lambda (x)
                                (let ((new-amount (+ x amount)))
                                  (if (< new-amount 0)
                                      0
                                    new-amount)))
                              color-list)))
...
(defun make-color-name (prefix name diff)
  "Create a color name: `PREFIX'-`NAME'`DIFF'."
  (concat prefix "-" name diff))
...
(defvar zenburn-bg-colors-alist
  (make-zenburn-color-alist "bg" "#202020" '("-2" "-1" "-05" nil "+05" "+1" "+2" "+3")))

(defvar zenburn-fg-colors-alist
  (make-zenburn-color-alist "fg" "#DCDCCC" '("+1" nil "-1")))

(defvar zenburn-red-colors-alist
  (make-zenburn-color-alist "red" "#CC9393" '("+1" nil "-1" "-2" "-3" "-4")))

(defvar zenburn-orange-colors-alist
  (make-zenburn-color-alist "orange" "#DFAF8F" '(nil)))

(defvar zenburn-yellow-colors-alist
  (make-zenburn-color-alist "yellow" "#F0DFAF" '(nil "-1" "-2")))

(defvar zenburn-green-colors-alist
  (make-zenburn-color-alist "green" "#7F9F7F" '("-1" nil "+1" "+2" "+3" "+4")))

(defvar zenburn-cyan-colors-alist
  (make-zenburn-color-alist "cyan" "#93E0E3" '(nil)))

(defvar zenburn-blue-colors-alist
  (make-zenburn-color-alist "blue" "#8CD0D3" '("+1" nil "-1" "-2" "-3" "-4" "-5")))

(defvar zenburn-magenta-colors-alist
  (make-zenburn-color-alist "magenta" "#DC8CC3" '(nil)))

(defvar zenburn-colors-alist
  (append zenburn-bg-colors-alist
          zenburn-fg-colors-alist
          zenburn-red-colors-alist
          zenburn-orange-colors-alist
          zenburn-yellow-colors-alist
          zenburn-green-colors-alist
          zenburn-cyan-colors-alist
          zenburn-blue-colors-alist
          zenburn-magenta-colors-alist))
...
;;;;; company-mode
   ...
   `(company-tooltip-selection ((t (:foreground ,zenburn-fg :background ,zenburn-bg+05))))
   `(company-tooltip-mouse ((t (:background ,zenburn-bg+05))))
   ...
   `(company-scrollbar-bg ((t (:background ,zenburn-bg+05))))
   `(company-preview ((t (:background ,zenburn-green+1))))
   `(company-preview-common ((t (:foreground ,zenburn-green+2 :background ,zenburn-bg+05))))
;;;;; bm
